...
export class AllRequestsComponent implements OnInit {
  ...
  getAllRequests() {
    ...
    this.spinner.show();
    this.api.post(APICollection.getAllRequestListAPI, payload).subscribe(
      response => {
        if (response.success) {
  ...
  onSubmit(data: any, status: string) {
    ...
    if (!data.assignparty && status !== 'REJECT') {
      return this.msgService.error('Please select party first.!');
    }
