...
@Component({
  selector: 'app-hawala-transactions',
  templateUrl: './hawala-transactions.component.html',
  styleUrls: ['./hawala-transactions.component.scss']
})
export class HawalaTransactionsComponent implements OnInit {
  ...
  getTransactions() {
    ...
    this.spinner.show();
    this.api.post(APICollection.getHawalaTransactionListAPI, payload).subscribe(
      response => {
        if (response.success) {
